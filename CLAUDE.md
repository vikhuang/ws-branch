...
| contrarian_broker | 5.54 | 4.37 | **3.36** | ✅ 真 alpha |
| dual_window | 4.68 | 3.11 | **2.51** | ✅ 真 alpha（*共用 conviction filter） |
| contrarian_smart | 2.43 | 0.64 | **-0.20** | ❌ 去重後 alpha 消失 |
| exodus | -1.03 | -0.53 | **-0.15** | ❌ 本來就沒有 |

- 1d-5d **全部策略 excess Sharpe 為負** — 短期交易成本吃掉 alpha
- **真正有 alpha 的獨立信號：conviction + concentration**（contrarian_broker 有 alpha 但共用 conviction filter）
...
### Signal Strength Validation（進行中）

**原則**：signal_value 不得在未驗證的情況下用於 position sizing。

filter 輸出 `signal_value=1.0`（uniform）+ `signal_count`（metadata）。
需先通過 quintile analysis 驗證「count 越高 → return 越好」才能啟用加權。

要驗證的 filter（有 alpha 的）：
- `filter_conviction_signals` → `signal_count = n_conviction`（conviction/contrarian_broker/dual_window 共用）
- `filter_concentration_increase` → `signal_count = n_adding`

n_conviction 分佈（200 股取樣）：n=3 佔 32%、n=4-5 佔 34%、n≥6 佔 34%，變異足夠。

**Quintile 結果（2026-03-16）**：
- signal_count: conviction ρ=+0.056（太弱），concentration ρ=+0.176（中等但覆蓋窄）
- churn_ratio: conviction ρ=-0.022, concentration ρ=-0.052（**反向！**高 churn = 逆勢加碼 = return 更好）

**結論：維持 signal_value = 1.0（uniform）。** 兩個候選指標都不足以 justify 加權。
churn ratio 反向的原因：filter 已篩出「加碼 broker」，高 churn 代表其他人反向操作 = 更強資訊優勢。
