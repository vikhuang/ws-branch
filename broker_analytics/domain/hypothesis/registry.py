...
STRATEGIES: dict[str, HypothesisConfig] = {
    ...
    "contrarian_broker": HypothesisConfig(
        ...
        display_name="反差券商",
        description="Contrarian brokers (high local, low global PNL) adding to winning positions",
        ...
        params={
            ...
            "train_end_date": "2023-12-31",
            "test_start_date": "2024-01-01",
            "min_global_amount": 1e9,
            "min_profit_ratio": 0.15,
            "min_brokers": 2,
        },
    ...
    "dual_window": HypothesisConfig(
        name="dual_window",
        display_name="雙窗口交集",
        description="Persistent winners (1yr ∩ 3yr top-K) adding to winning positions",
        selector=select_dual_window_intersection,
        filter=filter_conviction_signals,
        outcome=outcome_forward_returns,
        baseline=baseline_unconditional,
        stat_test=stat_test_permutation,
        params={"top_k": 20, "short_years": 1, "long_years": 3, "min_profit_ratio": 0.15, "min_brokers": 2},
        requires=frozenset({"trade_df", "pnl_daily_df", "prices"}),
    ),
    ...
    "ta_regime": HypothesisConfig(
        ...
        baseline=baseline_unconditional,
        stat_test=stat_test_permutation,
        params={"window_days": 120, "z_threshold": 1.0, "sigma": 2.0},
        requires=frozenset({"trade_df", "prices"}),
    ),
    ...
    "concentration": HypothesisConfig(
        ...
        baseline=baseline_unconditional,
        stat_test=stat_test_permutation,
        params={"min_concentration": 0.08, "min_brokers": 1},
        requires=frozenset({"trade_df", "pnl_daily_df", "prices"}),
    ),
