"""Generate 5-day rolling PNL ranking xlsx for 5 stocks.

- 2023-01 ~ 2025-12: monthly snapshots (last trading day of each month)
- 2026-01 ~ 2026-02: daily snapshots (every trading day)
...
def compute_rolling_ranking(
    ...
    """Compute 5-day rolling PNL ranking at a specific date."""
    # Find the index of snapshot_date in all_dates
    idx = None
...
def process_stock(symbol: str, stock_name: str, broker_names: dict[str, str]):
    ...
    out_dir = OUTPUT_ROOT / f"{symbol}_5d_windows"
    out_dir.mkdir(parents=True, exist_ok=True)

    count = 0
    for snap_date in snapshot_dates:
        ranking = compute_rolling_ranking(df, all_dates, snap_date, broker_names)
        if len(ranking) == 0:
            continue
    ...
    print(f"  {symbol} ({stock_name}): {count} files → {out_dir}")
