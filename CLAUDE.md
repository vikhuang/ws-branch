...
### Signal Strength Validation（完成）

**原則**：signal_value 不得在未驗證的情況下用於 position sizing。
filter 輸出 `signal_value=1.0`（uniform）+ `signal_count` + `churn_ratio`（metadata）。

**Quintile 結果（2026-03-16）**：

| 指標 | conviction ρ | concentration ρ | 可用？ |
|------|-------------|----------------|--------|
| signal_count（n_conviction） | +0.056 | +0.176 | ⚠ 太弱 |
| churn_ratio（低=共識） | **-0.022** | **-0.052** | ❌ 反向 |
...
1. **Alpha 來自行為本身**：浮盈加碼 = disposition effect 的反面。3 家 broker 這樣做就足以排除噪音。第 4-15 家增加的是「共識」不是「資訊」，而共識 = 擁擠 = 邊際 alpha 遞減。
2. **Selector 做了大部分工作**：策略的預測力來自「誰被選中」（rolling PNL top-K），不是「多少人通過 filter」。contrarian_broker 有最高 excess Sharpe，因為它的 selector 更精準。
3. **信號在 return variance 中佔比極低**：ρ=0.06 ≈ 0.4% variance explained。任何 magnitude 指標在這個雜訊水平下都難以突破。
