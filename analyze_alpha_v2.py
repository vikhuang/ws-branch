...
- Long: Alpha = (sell_price - end_price) / buy_price
  - Positive: sold before drop (good timing)
  - Negative: sold before rise (missed upside)

- Short: Alpha = (end_price - sell_price) / buy_price
  - Positive: covered before rise (good timing)
  - Negative: covered before drop (missed downside profit)

Alpha is weighted by trade size (shares × buy_price).
"""
...
def main():
    ...
    # Load price data for end price
    price_df = pl.read_parquet("price_master.parquet")
    end_date = price_df["date"].max()
    end_price = price_df.filter(pl.col("date") == end_date)["close_price"].item()

    # Load broker names
    with open("broker_names.json") as f:
        broker_names = json.load(f)

    print(f"期末日期：{end_date}")
    print(f"期末股價：{end_price:.1f}")
    ...
    trades_df = trades_df.with_columns([
        ...
        # If-held return (for long: if held to end; for short: if stayed short to end)
        pl.when(pl.col("trade_type") == "long")
        .then((end_price - pl.col("buy_price")) / pl.col("buy_price"))
        .otherwise((pl.col("buy_price") - end_price) / pl.col("buy_price"))
        .alias("hold_return"),
        ...
        (pl.col("trade_return") - pl.col("hold_return")).alias("alpha"),

        # Alpha in dollar terms
        ((pl.col("trade_return") - pl.col("hold_return")) * pl.col("trade_value")).alias("alpha_dollars"),
    ])
    ...
    # === Top Alpha ===
    print(f"\n{'='*90}")
    print("【Alpha 排行 Top 20】（正 Alpha = 賣/平倉時機優於持有）")
    print(f"{'='*90}")
    ...
    # === Bottom Alpha ===
    print(f"\n{'='*90}")
    print("【Alpha 排行 Bottom 20】（負 Alpha = 賣/平倉時機劣於持有）")
    print(f"{'='*90}")
    print(f"\n{'排名':<4} {'券商':<10} {'名稱':<18} {'Alpha':>10} {'Alpha金額':>14} {'交易額':>12} {'平倉筆數':>10}")
    print("-" * 90)
    ...
    print(f"Alpha > 0（擇時優於持有）：{positive_alpha} ({positive_alpha/len(alphas)*100:.1f}%)")
    print(f"Alpha < 0（擇時劣於持有）：{len(alphas) - positive_alpha} ({(len(alphas)-positive_alpha)/len(alphas)*100:.1f}%)")
    print(f"平均 Alpha：{sum(alphas)/len(alphas)*100:.2f}%")
    print(f"中位數 Alpha：{sorted(alphas)[len(alphas)//2]*100:.2f}%")
    ...
    for broker in focus_brokers:
        ...
        print(f"  Alpha：{row['weighted_alpha']*100:+.2f}%")
        print(f"  Alpha 金額：{row['total_alpha_dollars']/1e8:+.2f} 億")

        if row['weighted_alpha'] > 0:
            print(f"  → 擇時能力：優於 Buy & Hold")
        else:
            print(f"  → 擇時能力：劣於 Buy & Hold（不如不賣）")
    ...
    top_trades = trades_df.filter(pl.col("broker") == top_broker).sort("sell_date")
    ...
    # Sample trades
    print(f"\n最近 10 筆平倉：")
    print(f"{'賣出日':<12} {'買入日':<12} {'張數':>8} {'買價':>8} {'賣價':>8} {'報酬':>8} {'Alpha':>8}")
    print("-" * 75)

    for row in top_trades.tail(10).iter_rows(named=True):
        print(f"{row['sell_date']:<12} {row['buy_date']:<12} "
              f"{row['shares']:>8,} {row['buy_price']:>8.1f} {row['sell_price']:>8.1f} "
              f"{row['trade_return']*100:>+7.1f}% {row['alpha']*100:>+7.1f}%")
